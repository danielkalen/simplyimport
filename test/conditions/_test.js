...
exec('node '+__dirname+'/../../simplyimport.js -i '+__dirname+'/_importer.js -c yes yes1 -s', function(error, stdout, stderror){
	...
	if (stdout.match(/Imported file without extension/)) {console.log('Imported file without extension when failed to match 1 condition'); passedTests = false;}
	if (!stdout.match(/Imported nested level 1/)) {console.log('Failed to import nested file at level 1'); passedTests = false;}
	if (stdout.match(/Imported nested level 2/)) {console.log('Imported file nested file when failed to match all condition'); passedTests = false;}
