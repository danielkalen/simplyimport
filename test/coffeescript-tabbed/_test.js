...
exec('node '+__dirname+'/../../bin/simplyimport -i '+__dirname+'/_importer.coffee -s', function(error, stdout, stderror){
	...
	if (!stdout.match(/\s{4}\(\'Imported file without extension/)) {console.log('\x1b[31mFailed To Import file without extension\x1b[0m'); passedTests = false;}
	if (!stdout.match(/\t{2}\(\'Imported nested level 1/)) {console.log('\x1b[31mFailed To Import nested file at level 1\x1b[0m'); passedTests = false;}
