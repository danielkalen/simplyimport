...
exec('node '+__dirname+'/../../bin/simplyimport -i '+__dirname+'/_importer.coffee -s', function(error, stdout, stderror){
	...
	if (!stdout.match(/\('Imported nested level 1'\)\;/)) {console.log('\x1b[31mFailed to properly import quoted JS file to match proper coffeescript syntax\x1b[0m'); passedTests = false;}
	...
	if (passedTests) {
		console.log('Both (Coffee w/JS) Import - \x1b[32mPassed All Tests\x1b[0m');
	}
});