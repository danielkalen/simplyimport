...
exec('node '+__dirname+'/../../simplyimport.js -i '+__dirname+'/_importer.coffee -s', function(error, stdout, stderror){
	...
	if (!stdout.match(/\('Imported nested level 1'\)\;/)) {console.log('Failed to properly import quoted JS file to match proper coffeescript syntax'); passedTests = false;}
	...
	if (passedTests) {
		console.log('Both (Coffee w/JS) Import - Passed All Tests');
	}
});